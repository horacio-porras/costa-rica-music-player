(() => {
    ...
    Object.assign(Home, {
        registrarEventosPlaylists() {
            const self = this;

            $(document).on('click', '.home-library-item', function (event) {
                if ($(event.target).closest('.home-library-item-actions, .home-library-item-action-btn').length) return;
                if ($(event.target).closest('.home-library-item-cover-wrap').length) return;
            ...
            $(document).on('click', '#homePlaylistDetailContainer .playlist-song-row', function (e) {
                if ($(e.target).closest('.playlist-track-play-btn, .playlist-row-actions, .playlist-more-btn, .dropdown-menu').length) return;
                const row = $(this);
                ...
                if (song && window.Player) window.Player.reproducir(song, playlistSongs);
            });
        ...
        cargarPlaylists() {
            return $.get('/Playlists/ObtenerPlaylists', result => {
                if (result.esCorrecto) this.playlists = result.data || [];
            }).fail((xhr) => {
                if (xhr && xhr.status === 401) this.isAuthenticated = false;
                this.playlists = [];
            });
        },
        ...
        mostrarSidebarPlaylists() {
            ...
            this.playlists.forEach(playlist => {
                ...
                const coverMarkup = usarCollage
                    ? `<div class="home-library-cover-collage">${collageSources.map(src => `<img src="${src}" class="home-library-cover-collage-item" alt="${playlist.name}" onerror="this.onerror=null;this.src='/img/placeholder-cover.png';" />`).join('')}</div>`
                    : `<img src="${coverImage}" alt="${playlist.name}" class="home-library-item-cover" onerror="this.onerror=null;this.src='/img/placeholder-cover.png';" />`;

                list.append(`
                    <div class="home-library-item" data-playlist-id="${playlist.playlistId}">
                        <div class="home-library-item-cover-wrap">
                            ${coverMarkup}
                            <span class="home-library-play-overlay" aria-hidden="true"><i class="fas fa-play"></i></span>
                        </div>
                        ...
                        <div class="home-library-item-actions">
                            <button type="button" class="home-library-item-action-btn btn-edit" title="Editar playlist" aria-label="Editar playlist"><i class="fas fa-pen"></i></button>
                            <button type="button" class="home-library-item-action-btn btn-delete" title="Eliminar playlist" aria-label="Eliminar playlist"><i class="fas fa-trash"></i></button>
                        </div>
                    </div>
                `);
            });
        ...
        actualizarContadorPlaylistSidebar(playlistId) {
            ...
            if (item.length > 0) item.find('.home-library-item-meta').text(`${playlist.songCount} canciones`);
        },
        ...
        obtenerCancionDesdeFila(row, playlistSongs) {
            ...
            if (songById || songByIndex) return songById || songByIndex;
            ...
            const duration = this.parsearDuracion(row.find('td.text-end').first().text().trim());
            return { songId, title, artistName, coverImageUrl, duration, filePath: '' };
        },
        ...
        async mostrarAvisoLoginCrearPlaylist() {
            if (typeof Swal !== 'undefined') {
                const result = await Swal.fire({
                    ...
                    customClass: { popup: 'playlist-swal-popup', confirmButton: 'playlist-swal-confirm', cancelButton: 'playlist-swal-cancel' },
        ...
        cargarPlaylistEnModalEditar(playlistId) {
            $.get(`/Playlists/ObtenerPlaylistPorId?id=${playlistId}`, result => {
                ...
                if (typeof window.cargarPlaylistEnModal === 'function') window.cargarPlaylistEnModal(result.data);
                $('#modalEditarPlaylist').modal('show');
            }).fail(() => alert('Error al cargar la playlist.'));
        },
        ...
        recargarSidebarYDetalle(playlistIdPreferido = null) {
            this.cargarPlaylists().then(() => {
                ...
                if (detalleVisible) this.cargarDetallePlaylistEmbebido(targetId);
            });
        },
        ...
        async cargarDetallePlaylistEmbebido(playlistId) {
            ...
            try {
                ...
                if (!result || !result.esCorrecto || !result.data) return;
        ...
        async reproducirPlaylistDesdeSidebar(playlistId) {
            ...
            try {
                ...
                if (!songs.length || typeof window.Player === 'undefined') return;
        ...
        async agregarCancionDesdeModal(playlistId, songId, btnElement) {
            ...
            try {
                const body = new URLSearchParams({ playlistId: String(playlistId), songId: String(songId) });
                ...
                if (result && result.esCorrecto) {
                    ...
                    document.dispatchEvent(new CustomEvent('playlist-song-added', { detail: { playlistId, songId } }));
                    if (Number(this.selectedPlaylistId) === Number(playlistId)) this.cargarDetallePlaylistEmbebido(playlistId);
                    return;
                }
                const mensaje = (result && result.mensaje) ? result.mensaje : 'No se pudo agregar la canción.';
                if (typeof Swal !== 'undefined') {
                    await Swal.fire({ title: 'No se pudo agregar', text: mensaje, icon: 'error', background: '#232323', color: '#f3f3f3', confirmButtonColor: '#1db954' });
                } else {
                    alert(mensaje);
                }
            } catch (error) {
                if (typeof Swal !== 'undefined') {
                    await Swal.fire({ title: 'Error de conexión', text: 'No se pudo conectar con el servidor.', icon: 'error', background: '#232323', color: '#f3f3f3', confirmButtonColor: '#1db954' });
                } else {
        ...
        renderizarDetallePlaylistEmbebido(playlist) {
            ...
            const collageMarkup = collage.length > 0
                ? [0, 1, 2, 3].map(i => {
                    ...
                    return `<img src="${src}" class="playlist-collage-item" alt="thumbnail cancion ${i + 1}" onerror="this.onerror=null; this.src='/img/placeholder-cover.png';" />`;
                }).join('')
                    ...
                    return `
                        <tr class="playlist-song-row" data-song-index="${index}" data-song-id="${songId}">
                            <td class="text-secondary playlist-track-cell">
                                ...
                                <button type="button" class="playlist-track-play-btn" title="Reproducir ${this.escapeHtml(title)}"><i class="fas fa-play"></i></button>
                            </td>
                            <td>
                                <div class="playlist-song-title-cell">
                                    <img src="${coverImageUrl}" alt="${this.escapeHtml(title)}" class="playlist-song-cover" onerror="this.onerror=null; this.src='/img/placeholder-cover.png';" />
                            ...
                            <td class="playlist-row-actions text-end">
                                <button type="button" class="playlist-more-btn" title="Mas opciones" aria-label="Mas opciones" data-bs-toggle="dropdown" aria-expanded="false"><i class="fas fa-ellipsis-h"></i></button>
                                <ul class="dropdown-menu dropdown-menu-end playlist-song-dropdown">
                                    <li><button type="button" class="dropdown-item playlist-remove-song-option" data-playlist-id="${playlistId}" data-song-id="${songId}"><i class="fas fa-trash me-2"></i>Eliminar de esta playlist</button></li>
                                </ul>
                            </td>
                        </tr>`;
                }).join('');
                ...
                <div class="playlist-detail-page home-embedded-detail">
                    <div class="playlist-detail-header">
                        <button type="button" class="playlist-detail-cover is-editable js-playlist-cover-edit" data-playlist-id="${playlistId}" title="Editar playlist" aria-label="Editar playlist">
                            ${usaPortadaSeleccionada
                                ? `<img src="${cover}" alt="${this.escapeHtml(playlist.name)}" class="playlist-detail-cover-image" onerror="this.onerror=null; this.src='/img/placeholder-cover.png';" />`
                                : `<div class="playlist-collage">${collageMarkup}</div>`}
                            <span class="playlist-cover-edit-overlay"><i class="fas fa-pen"></i></span>
                        </button>
                    ...
                    <div class="playlist-detail-body">
                        <div class="playlist-detail-toolbar d-flex justify-content-between align-items-center">
                            <button type="button" class="btn-playlist-back" id="btnVolverHomeDiscovery"><i class="fas fa-arrow-left me-2"></i>Volver</button>
                            <button type="button" class="btn-spotify btn-sm js-add-songs-to-playlist" data-playlist-id="${playlistId}" data-playlist-name="${this.escapeHtml(playlist.name)}"><i class="fas fa-plus me-2"></i>Agregar canciones</button>
                        </div>
                        <div class="playlist-songs-table-wrapper">
                            <table class="table playlist-songs-table align-middle mb-0">
                                <thead><tr><th style="width: 50px;">#</th><th>Titulo</th><th>Album</th><th class="text-end" style="width: 90px;"><i class="far fa-clock"></i></th><th style="width: 44px;"></th></tr></thead>
        ...
        async aplicarDegradadoHeaderPlaylist(containerElement) {
            ...
            if (!header) return;
            const sourceImage = header.querySelector('.playlist-detail-cover-image') || header.querySelector('.playlist-collage-item');
            const src = sourceImage ? sourceImage.getAttribute('src') : null;
            if (!src) return;
            try {
                const color = await this.obtenerColorDominante(src);
                if (!color) return;
                const dark = { r: Math.max(0, Math.round(color.r * 0.45)), g: Math.max(0, Math.round(color.g * 0.45)), b: Math.max(0, Math.round(color.b * 0.45)) };
        ...
        async eliminarCancionDePlaylistEmbebida(playlistId, songId) {
            ...
            try {
                ...
                if (typeof Swal !== 'undefined') {
                    await Swal.fire({ title: 'No se pudo eliminar', text: mensaje, icon: 'error', background: '#232323', color: '#f3f3f3', confirmButtonColor: '#1db954' });
                } else {
                    alert(mensaje);
                }
            } catch (error) {
                if (typeof Swal !== 'undefined') {
                    await Swal.fire({ title: 'Error de conexion', text: 'No se pudo conectar con el servidor.', icon: 'error', background: '#232323', color: '#f3f3f3', confirmButtonColor: '#1db954' });
                } else {
                    alert('No se pudo conectar con el servidor.');
                }
            }
